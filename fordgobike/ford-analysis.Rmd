...
Given the increased usage during commute hours, one may wonder if any ride pattern exists and can be detected at those times. We will focus our investigation on rides in San Francisco. Before diving in, here are all the active stations in the city:
...
Docks are peppered throughout San Francisco with a fairly homogeneous spread. At the heart of the coverage area is South of Market, home to Uber and Twitter Headquarters. Now that we have a sense of the bike station locations, we will move forward with the goal of visualizing the start and end points of bike trips at a specific hour. This was implemented using the code below:
...
df_transit = data.frame(name = c("Embarcadeo BART Station",   # station names
                                 ...
                                 "Ferry Building"),
                        system = c(rep("BART", 6), rep("Caltrain", 2), "Ferry"),   # transit system
