...
* Bike sharing is likely a secondary means of transportation behind the major transit systems, such as the Bay Area Rapid Transit, Caltrain, and San Francisco Ferry.
...
```{r warning=FALSE, message=FALSE}
# Function for visualizing starting and ending station usage at a designated time
# Filter for rides taken at 8 AM and grouped by start time and start station
peak_AM_start = df %>%
  group_by(start_hour, start_station_longitude, start_station_latitude) %>%
  rename(station_longitude = start_station_longitude, station_latitude = start_station_latitude) %>%
  summarise(rides = dplyr::n()) %>%
...
peak_AM_end = df %>%
  ...
  rename(station_longitude = end_station_longitude, station_latitude = end_station_latitude) %>%
  summarise(rides = dplyr::n()) %>%
  filter(start_hour == "08") %>%
  mutate(type = "End Stations")

# Combine starting and ending stations into one data frame
peak_AM = bind_rows(peak_AM_start, peak_AM_end)

# Convert "type" to factor
peak_AM$type = factor(peak_AM$type, 
                      levels = c("Start Stations", "End Stations"))

# Plot start stations
ggmap(SFmap) +
  geom_point(data = peak_AM, 
